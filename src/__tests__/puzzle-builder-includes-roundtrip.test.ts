...
describe('PuzzleBuilder - Include Files Round-trip', () => {
  const includesDir = path.join(__dirname, '../chialisp/base');
  ...
  (list AGG_SIG_ME CREATE_COIN ASSERT_MY_COIN_ID)
      ...
      const tempFile = path.join(os.tmpdir(), `test-condition-codes-${Date.now()}.clsp`);
  ...
  (assert (> x 0) (> y 0))
      ...
      const tempFile = path.join(os.tmpdir(), `test-assert-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  (list
    ...
    (and (> a 0) (> b 0) (> c 0))
      ...
      const tempFile = path.join(os.tmpdir(), `test-or-and-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  describe('curry-and-treehash.clinc', () => {
  ...
  (puzzle-hash-of-curried-function function_hash param2 param1)
      ...
      const tempFile = path.join(os.tmpdir(), `test-curry-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
    ...
    test('should preserve function definition order', () => {
      const filePath = path.join(includesDir, 'curry-and-treehash.clinc');
      const content = fs.readFileSync(filePath, 'utf-8');
  ...
  (list
    ...
    (singleton_launcher_id_truth truths)
      ...
      const tempFile = path.join(os.tmpdir(), `test-singleton-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  (list
    ...
    (cat_tail_program_hash_truth truths)
      ...
      const tempFile = path.join(os.tmpdir(), `test-cat-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  (sha256tree data)
      ...
      const tempFile = path.join(os.tmpdir(), `test-sha256tree-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  (list
    ...
    (list ASSERT_MY_COIN_ID (my_id_truth truths))
      ...
      const tempFile = path.join(os.tmpdir(), `test-multi-include-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  (puzzle-hash-of-curried-function 
    ...
    param1
      ...
      const tempFile = path.join(os.tmpdir(), `test-nested-include-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  CREATE_COIN
      ...
      const tempFile = path.join(os.tmpdir(), `test-ast-condition-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
  ...
  (assert 
    ...
    (or (= z 1) (= z 2))
      ...
      const tempFile = path.join(os.tmpdir(), `test-ast-macro-${Date.now()}.clsp`);
      fs.writeFileSync(tempFile, testMod);
