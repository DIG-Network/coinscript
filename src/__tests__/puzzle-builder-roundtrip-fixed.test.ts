/**
 * Round-trip Tests for ChiaLisp and CoinScript (Fixed)
 * 
 * Tests that verify:
 * 1. ChiaLisp -> AST -> ChiaLisp produces functionally equivalent output
 * 2. ChiaLisp -> AST and CoinScript -> AST produce equivalent AST structures
 * 
 * Note: The ChiaLisp parser converts uppercase variable names to hex values
 * and the serializer doesn't preserve exact formatting, so we test for
 * functional equivalence rather than exact string matching.
 */
...
describe('PuzzleBuilder - Round-trip Tests (Fixed)', () => {
  ...
  describe('ChiaLisp to AST Round-trip', () => {
    ...
    test('mod with if-then-else', () => {
      ...
      expect(exported).toContain('(if (> x 10) 100 200)');
    });
    ...
    test('complex real-world puzzle with uppercase params', () => {
  ...
  (create_transfer_conditions new_owner amount)
      ...
      expect(exported).toContain('CREATE_COIN');
      expect(exported).toContain('AGG_SIG_ME');
      // OWNER_PUBKEY is converted to hex
      expect(exported).toMatch(/0x[0-9a-f]+/);
    });
  ...
  (list
    ...
    test('real-world singleton pattern with uppercase params', () => {
  ...
  (check_and_morph_conditions (a INNER_PUZZLE inner_solution) SINGLETON_STRUCT)
      ...
      // Uppercase params are converted to hex
      expect(exported).toMatch(/0x[0-9a-f]+/);
      expect(exported).toContain('inner_solution');
    });
  ...
  (if (> x 0)
    ...
    (if (< x 0)
      ...
      expect(ifCount).toBeGreaterThanOrEqual(7); // At least 7 if statements
