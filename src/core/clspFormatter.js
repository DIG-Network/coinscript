...
exports.CLSPFormatter = void 0;
exports.formatCLSP = formatCLSP;
// Token types
var TokenType;
(function (TokenType) {
...
class CLSPFormatter {
    ...
    parseExpression(tokens, startIdx = 0) {
        let idx = startIdx;
    ...
    getSpecialIndent(specialForm) {
        // Special indentation rules based on form
        const specialIndents = {
    ...
    formatDefconstant(expr, _indent) {
    ...
    tryFormatOneLine(expr, _indent) {
        ...
        const hasMultiLine = elements.some(elem => elem.type === 'list' && this.tryFormatOneLine(elem, 0) === null);
        if (hasMultiLine)
            return null;
        return result;
    }
    formatMultiLine(expr, indent, specialIndent) {
        const result = ['('];
        const elements = expr.elements?.filter(e => e.type !== 'newline') || [];
        elements.forEach((elem, idx) => {
            if (idx === 0) {
    ...
    getExpressionType(expr) {
        ...
        if (expr.type === 'list' && expr.specialForm) {
            if (expr.specialForm === 'include')
                return 'include';
            if (expr.specialForm === 'defconstant')
                return 'defconstant';
            if (expr.specialForm === 'defun' || expr.specialForm === 'defun-inline')
                return 'defun';
            if (expr.specialForm === 'defmacro')
                return 'defmacro';
            if (expr.specialForm === 'mod')
                return 'mod';
        }
    ...
    needsBlankLineBefore(lastType, currentType) {
        // Always add blank line before defun/defmacro
        if (currentType === 'defun' || currentType === 'defmacro')
            return true;
        // Add blank line when transitioning between different groups
        if (lastType === 'include' && currentType !== 'include')
            return true;
        if (lastType === 'defconstant' && currentType !== 'defconstant')
            return true;
        // Add blank line after defun/defmacro
        if ((lastType === 'defun' || lastType === 'defmacro') && currentType !== 'comment')
            return true;
        return false;
    }
}
exports.CLSPFormatter = CLSPFormatter;
/**
 * Format CLSP code with default options
 */
function formatCLSP(content, options) {
