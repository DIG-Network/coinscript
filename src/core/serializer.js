...
function serialize(node, options = {}) {
    const { hexPrefix = true, indent = false, indentString = '  ', blockComments = [], includedLibraries = [] } = options;
    // Ensure comments is properly typed
    const comments = options.comments || new Map();
    return serializeNode(node, 0);
    function serializeNode(n, depth, skipComment = false) {
        let result = '';
        ...
        if ((0, types_1.isAtom)(n)) {
            return serializeAtom(n.value) + result;
        }
        else if ((0, types_1.isList)(n)) {
            return serializeList(n.items, depth, n) + result;
        }
        else if ((0, types_1.isCons)(n)) {
            return serializeCons(n, depth) + result;
        }
        throw new types_1.SerializeError(`Unknown node type: ${JSON.stringify(n)}`);
    }
    function serializeAtom(value) {
        // Nil
        if (value === null) {
            return '()';
        }
        ...
        throw new types_1.SerializeError(`Cannot serialize atom value: ${String(value)}`);
    }
    function serializeList(items, depth, _node) {
        if (items.length === 0)
            return '()';
        if (indent) {
            ...
            if ((0, types_1.isAtom)(firstItem) && firstItem.value === 'mod' && items.length >= 3) {
                ...
                if ((0, types_1.isList)(params) && params.items.length > 3) {
            ...
            if ((0, types_1.isAtom)(firstItem) && ['defun', 'defun-inline', 'defmacro'].includes(firstItem.value) && items.length >= 3) {
                const form = firstItem.value;
            ...
            // Handle if specially
            if ((0, types_1.isAtom)(firstItem) && firstItem.value === 'if' && items.length >= 3) {
            ...
            if ((0, types_1.isAtom)(firstItem) && firstItem.value === 'list' && items.length > 2) {
                let result = '(list';
            ...
            if ((0, types_1.isAtom)(firstItem) && firstItem.value === 'include') {
                return `(${items.map(item => serializeNode(item, 0)).join(' ')})`;
            }
            // Handle cons (c) operations - check if we should inline
            if ((0, types_1.isAtom)(firstItem) && firstItem.value === 'c' && items.length === 3) {
            ...
            const allSimple = items.every(item => (0, types_1.isAtom)(item) || ((0, types_1.isList)(item) && item.items.length <= 3));
    ...
    function isInclude(node) {
        return (0, types_1.isList)(node) && node.items.length >= 2 &&
            (0, types_1.isAtom)(node.items[0]) && node.items[0].value === 'include';
    }
    // Helper to get parameter comments (would need more context in real implementation)
    function getCommentForParam(param) {
 ...
 */
function getConditionCodeName(value) {
 ...
 */
function getSymbolReplacement(symbol, includedLibraries) {
